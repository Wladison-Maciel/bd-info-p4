...
db.serialize(function() {
    db.run("CREATE TABLE E_Faixa_Horaria (id INTEGER PRIMARY KEY AUTOINCREMENT, FaixaHoraria ENUM('primeiro_horario', 'segundo_horario', 'terceiro_horario', 'quarto_horario', 'quinto_horario'))");
  
    let stmt = db.prepare("INSERT INTO E_Faixa_Horaria (FaixaHoraria) VALUES (?)");
    stmt.run('primeiro_horario');
    stmt.run('segundo_horario');
    stmt.run('terceiro_horario');
    stmt.run('quarto_horario');
    stmt.run('quinto_horario');
    stmt.finalize();
  
    db.each("SELECT id, FaixaHoraria FROM E_Faixa_Horaria", function(err, row) {
        console.log(row.id + ": " + row.FaixaHoraria);
    });
  });
...
db.run(
    'CREATE TABLE IF NOT EXISTS TB_NOTA_FISCAL (id INTEGER PRIMARY KEY, valor FLOAT, cliente_id INTEGER, vendedor_id INTEGER, foreign key (cliente_id) references TB_CLIENTES (id), foreign key (vendedor_id) references TB_CAMPUS (id))',
    (err) => {
        if (err) {
...
db.run(
    'CREATE TABLE IF NOT EXISTS TB_LABORATORIO (id INTEGER PRIMARY KEY, descricao TEXT, preco_unitario FLOAT)',
    (err) => {
        if (err) {
